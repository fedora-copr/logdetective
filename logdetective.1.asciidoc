...
*-C* *N_CLUSTERS*, *--n-clusters* *N_CLUSTERS*::
  Number of clusters to use with the Drain summarizer. (optional, default 8)

*-n*, *--no-stream*::
  Disable streaming output of analysis results, i.e. print the whole response at once.

*-v*, *--verbose*::
  Enable verbose output during processing (use `-vv` or `-vvv` for higher levels of verbosity).

*-q*, *--quiet*::
  Suppress non-essential output (only leaving actual LLM explanations).
...
*--prompts-config* *PROMPTS*:: Path to prompt configuration file where you can customize (override default) prompts sent to the LLM and set optional configuration (default `logdetective/prompts.yml`). See https://github.com/fedora-copr/logdetective/blob/main/logdetective/prompts.yml for reference.

*--prompt-templates* *TEMPLATE_DIR*:: Path to prompt template dir (default `logdetective/prompts`). Prompts must be valid Jinja templates, and system prompts must include field *system_time*.

*--temperature* *TEMPERATURE*::
  Temperature for inference. Higher temperature leads to more creative and variable outputs. (default 0.0)

*--skip-snippets* *SNIPPETS_FILE*::
  Path to patterns for skipping snippets. User can specify regular expressions matching log chunks which do not contribute to the analysis. Patterns must be defined in a TOML file where each entry is a table with a `pattern` key (regex) and an optional `files` key listing exact filenames the pattern applies to. When `files` is omitted the pattern applies to every log file. Use single-quoted TOML strings for patterns to avoid any escaping issues.
  +
  Examples:
...
*--mib_limit* *NUMBER*::
  Limit the size (in MiB) of file (if submitting via URL) for LLM analysis (default 50). Logs exceeding this will be rejected.
...
With specific model from HuggingFace (`namespace/repo_name`, note that `--filename-suffix` is also needed):
