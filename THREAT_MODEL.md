...
Untrusted external text (`commentary`, and `infra_status` user inputs) is enclosed in XML tags
with null-replacing attempts to escape boundary break-outs using regex-checking.
Log artifacts, also externally provided, are processed inside tool calls,
with verbose framing to treat data extracted from artifacts as such,
and schema constraints are enforced on the payload. Other constraints enforced
by the agentic framework include structured Pydantic schema output validation
(AgentResponse), tool invocation limits (ConditionalRequirement),
and trajectory logging (GlobalTrajectoryMiddleware) for monitoring.
...
| T6 | Attacker submits crafted log content designed to manipulate LLM behavior (prompt injection), causing misleading analysis, fabricated solutions, or attacker-chosen text in explanations posted as GitLab MR comments | remote_unauth | POST /analyze, POST /webhook/gitlab/job_events | llm_analysis_responses | high | possible | partially_mitigated | Input sanitization (PII redacted); Drain3/csgrep/traceback extractors reduce attacker-controlled text reaching the LLM; chat-API role separation places log content in user message, instructions in system message; untrusted fields (commentary, infra status) are enclosed in XML tags with null-replacing attempts to escape boundaries (checked using regex); server path enforces `AgentResponse` Pydantic schema validation on LLM output (agent.py:172,188); tools extracting data from log artifacts contain additional security framing; tool invocation limits enforce execution bounds; trajectory logging helps with monitoring; agent tools are sandboxed to extraction logic only; MR comments carry AI disclaimer and abuse-reporting link | Attacker-controlled log content is embedded verbatim in the LLM prompt after snippet extraction; log artifact text processed inside tool calls is not wrapped in XML tags, instead tool's description informs it about working with external data; CLI path has no output validation; no prompt-injection-specific secondary classifier; human-facing text in explanations remains an unvalidated residual risk |
...
| T11 | Attacker sends specially crafted zip archives via GitLab artifacts, causing excessive memory consumption | remote_unauth / remote_auth | POST /webhook/gitlab/job_events | service_availability | medium | rare | partially_mitigated | Content-Length check on artifacts zip; `zipfile.open()` per entry (no `extractall()`); max_artifact_size configurable; chunked reading of individual entries with enforced limit; | On-disk TemporaryFile for zip download, individual entries decompressed into memory via `zipfile.open().read()` in chunks |
