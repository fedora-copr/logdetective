...
| koji_callback_url | X-Koji-Callback header value; server POSTs `{"task_id": N}` to this URL upon completion (no credentials, logs, or analysis data in payload) | application process → attacker-controlled server (no SSRF boundary) | — |
...
| T8 | Rejected authentication tokens and authorization headers are logged in plaintext at INFO/WARNING level and echoed in HTTP error response bodies; an attacker with access to server logs (via log file exposure, Sentry, centralized log aggregation, or OpenShift Logging) or network position to observe error responses recovers submitted credentials; log-aggregator exposure affects operators beyond the caller's own trust boundary | remote_unauth | POST /analyze, all authenticated endpoints, environment_and_config | api_auth_token, koji_auth_tokens | high | possible | partially_mitigated | In podman-compose, log file requires filesystem access; in OpenShift, logs are typically aggregated to a cluster-wide logging stack (Elasticsearch/Loki) accessible to wider set of operators; Sentry DSN optional | Token value logged at server.py:221 and returned in HTTP 401 detail at server.py:222; full authorization header logged at server.py:210 and returned in HTTP 401 detail at server.py:217; Koji token returned in response body at server.py:301,354 |
| T1 | Attacker exploits Koji callback URL to perform blind SSRF against internal services, reaching cloud metadata endpoints, Kubernetes API, or cluster-internal services; the callback sends only `{"task_id": N}` via POST with no credentials, so impact is limited to network probing and triggering side effects on endpoints that act on unauthenticated POST requests | remote_unauth / remote_auth | POST /analyze/rpmbuild/koji/{instance}/{task_id} via X-Koji-Callback header | service_availability | high | rare | partially_mitigated | Koji token auth required when configured (empty `tokens` list bypasses check); in podman-compose, container network namespace limits reachable targets; in OpenShift, NetworkPolicies restrict egress but cloud metadata and Kubernetes API may still be reachable; no SSRF protection on callback URL; response from target is discarded (blind SSRF) | `send_koji_callback()` creates a new `aiohttp.ClientSession` without `SSRFProtectedResolver` and sends only `json={"task_id": task_id}` — no Authorization header, no credentials in payload; response is discarded (`pass` in context manager body) (server.py:457-461) |
| T16 | Compromised application pod uses auto-mounted service account token to access the Kubernetes API, escalating from application-level compromise to cluster-level reconnaissance, lateral movement, or authenticated Secret retrieval; this is the only path from application compromise to credential exfiltration via the Kubernetes API, since unauthenticated requests (e.g., via blind SSRF in T1) are rejected | remote_auth | openshift_api | openshift_secrets, openshift_route_config, inference_api_credentials | high | rare | partially_mitigated | OpenShift restricts default service account permissions; SecurityContextConstraints (SCCs) limit pod capabilities | Service account token auto-mounted at /var/run/secrets/kubernetes.io/serviceaccount unless `automountServiceAccountToken: false` is set; Kubernetes API is reachable from any pod at kubernetes.default.svc; if the service account has `get secrets` permission, this token provides authenticated access to all Secrets in the namespace |
...
| T10 | Jinja2 templates rendered without autoescape could enable content injection if LLM output contains markup and is rendered in a web context; `{{ explanation }}` and `{{ package }}` are unescaped in both `gitlab_full_comment.md.j2` and `gitlab_short_comment.md.j2` while `snippet.text` and `snippet.snippet_analysis` use `\| e` | remote_unauth | POST /webhook/gitlab/job_events (MR comment posting) | llm_analysis_responses | medium | rare | partially_mitigated | GitLab renders markdown with its own sanitization; snippet text uses `\| e` escape filter; templates are developer-authored | `jinja2.Environment(loader=...)` without `autoescape=True` at gitlab.py:491; `{{ explanation }}` rendered unescaped in both MR comment templates |
...
|---|---|---|---|
| Apply `SSRFProtectedResolver` to the `aiohttp.ClientSession` used in `send_koji_callback()`, or restrict callback URLs to a configured allowlist of Koji instance domains; in OpenShift, add an egress NetworkPolicy denying access to 169.254.169.254 and kubernetes.default.svc from the application pod | T1, T16 | yes | S |
| Remove token values from log messages and HTTP error response bodies; log only "invalid token" without echoing the submitted value; includes Koji token in `BasicResponse` at server.py:301,354 | T8 | yes | S |
...
| Enable `autoescape=True` on Jinja2 environments, or apply the `\| e` filter to all user-influenced template variables including explanation text | T10 | partial | S |
...
| Configure OpenShift Route with re-encrypt TLS termination to maintain encryption between the Route and the application pod, preventing credential and log content interception on the intra-cluster network path | T8 | partial | S |
...
| Configure egress NetworkPolicies in OpenShift to restrict outbound traffic from the application pod to only required destinations (inference provider, GitLab, Koji, PostgreSQL) | T1, T16 | partial | M |
