...
| T6 | Attacker submits crafted log content designed to manipulate LLM behavior (prompt injection), causing misleading analysis, fabricated solutions, or attacker-chosen text in explanations posted as GitLab MR comments | remote_unauth | POST /analyze, POST /webhook/gitlab/job_events | llm_analysis_responses | high | possible | partially_mitigated | Input sanitization (PII redacted); Drain3/csgrep/traceback extractors reduce attacker-controlled text reaching the LLM; chat-API role separation places log content in user message, instructions in system message; untrusted fields (commentary, infra status) are enclosed in XML tags with null-replacing attempts to escape boundaries (checked using regex); server path enforces `AgentResponse` Pydantic schema validation on LLM output (agent.py:172,188); tool invocation limits enforce execution bounds; trajectory logging helps with monitoring; agent tools are sandboxed to extraction logic only; MR comments carry AI disclaimer and abuse-reporting link | Attacker-controlled log content is embedded verbatim in the LLM prompt after snippet extraction; log artifact text processed inside tool calls is not wrapped in XML tags; CLI path has no output validation; no prompt-injection-specific secondary classifier; human-facing text in explanations remains an unvalidated residual risk |
