...
and Koji (RPM build task analysis with callback notifications), storing metrics in PostgreSQL.
...
| GET /metrics/{route}/{metric_type} | Metrics endpoint returning request/response statistics, optionally filtered by non-secret token name; subject to global bearer auth like all other routes | remote unauth/auth peer → application process | database_records |
...
| T7 | Global bearer-token authentication is disabled when `LOGDETECTIVE_TOKENS_FILE` is unset; misconfigured production deployment exposes `/analyze`, `/version`, and `/metrics` without auth; Koji and GitLab webhook endpoints retain their independent header checks | local_admin | POST /analyze, GET /version, GET /metrics | llm_analysis_responses, database_records | high | possible | partially_mitigated | Documented behavior; invalid or missing configured token files fail startup; named token values are compared in constant time and are absent from errors and metrics | An operator can still omit both authentication environment variables |
